...
RegisterNUICallback(
    ...
    function(data)
        -- Do tablet hide shit
        if data.load then
            print("[CAT6] Loaded the tablet")
            tabLoaded = true
        elseif data.hide then
            print("[CAT6] Hiding the tablet")
...
Citizen.CreateThread(
    function()
        ...
        if timeout == true then
            print("[CAT6] Failed to load tablet nui...")
        -- return ---- Quit
        end

        print("[CAT6] ::The client lua for tablet loaded::")
        ...
        while true do
            ...
            if IsControlPressed(0, 19) and IsControlPressed(0, 170) and GetLastInputMethod( 0 ) then
                tabEnabled = not tabEnabled -- Toggle tablet visible state
                REQUEST_NUI_FOCUS(tabEnabled)
                print("[CAT6] The tablet state is: " .. tostring(tabEnabled))
                Citizen.Wait(0)
            end
            if (tabEnabled) then
