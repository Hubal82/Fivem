...
function OpenCloakroomMenu()
	...
	if Config.EnableNonFreemodePeds then
		...
		table.insert(elements, {label = 'Biker wear', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01'})
...
function GetAvailableVehicleSpawnPoint(station, partNum)
	local spawnPoints = Config.BikerStations[station].Vehicles[partNum].SpawnPoints
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'biker' and PlayerData.job.name == dispatchNumber then
